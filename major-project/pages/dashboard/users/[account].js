...
const Account = () => {
    ...
    const verifyStudent = async () => {
        ...
        try {
            const certi=await CrediContract.methods.GetCertificatesByStudent(fetchedUser?.address).call();
            if(certi.length > 0){
                ...
                toast.error("Student do not any verifiable certificate");
        } catch (error) {
            toast.error("Something went wrong.");
            console.log(error);
        }
    ...
    return (
        <>
            <Head>
                <title>Certi-Locker | Professors</title>
                    ...
                    {user?.role === "Company" && !valid && (
                        <Box my={2}>
                            <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
                                <Button onClick={requestCertificate} variant="contained" color="primary" disabled={disabled}>
                                    Request for Certificate
                                </Button>

                                <Button onClick={verifyStudent} variant="contained" color="primary">
                                    Verify Student
                                </Button>
                            </Box>
                        </Box>
